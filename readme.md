...
Приложение *gitsync* представляет собой отдельное (standalone) приложение на 1Script, и предназначено для синхронизации хранилища конфигураций 1С с репозиторием git.
...
Возможны следующие сценарии использования/организации хранения исходников в репозиториях Git:

1. в одном репозитории Git хранятся как исходники конфигурации 1С, так и прочие исходники/файлы продукта (например, документация, тесты и прочее) - `рекомендуется`
  + Используем этапы:
    + Клонировать средствами Git исходный репозиторий (`git clone`) или создать новый (`git init`)
    + `Выполнение настройки для начала выгрузки`
    + `Синхронизация` или `Экспорт исходников конфигурации в сторонний репозиторий Git`
    + далее штатные инструменты Git - `commit/pull/push` 
  
2. в одном репозитории хранятся только исходники конфигурации 1С - `не рекомендуется`
  +  Используем этапы: 
    + `Подготовка нового репозитория` - если репозитория Гит еще нет
    + `Клонирование существующего пустого репо` - если уже был создан пустой репозиторий Git
    + `Выполнение настройки для начала выгрузки`
    + `Синхронизация`

## Экспорт исходников конфигурации в сторонний репозиторий Git
Основной режим работы для сценария `в одном репозитории Git хранятся как исходники конфигурации 1С, так и прочие исходники/файлы продукта (например, документация, тесты и прочее)`.

+ Клонировать средствами Git исходный репозиторий (`git clone`) или создать новый (`git init`)
+ Перейти в каталог репозитория Git
...
+ `AUTHORS` - содержит информацию о связке пользователей хранилища 1С и пользователей репозитория Git

Указанные файлы должны находиться в корне каталога выгрузки исходников, который может не совпадать с корнем репозитория Git в зависимости от выбранного сценария организации хранения исходников.
...
## Синхронизация
Основной режим работы для сценария `Хранение исходников конфигурации 1С в отдельном репозитории Git`. Аргументы командной строки для запуска:

* <каталог или файл хранилища>
* <адрес (url) репозитория>
* [Каталог исходников внутри локальной копии git-репозитория]
...
## Команды
  ...
  ```
  Синхронизация хранилища конфигураций 1С с репозиторием GIT.
  Использование:
...
Необходимо добавить в `.gitignore` строку со значением `ConfigDumpInfo.xml`. Если такого файла нет в корне git-репозитория, необходимо создать его вручную, добавить строку `ConfigDumpInfo.xml` и закоммитить.
